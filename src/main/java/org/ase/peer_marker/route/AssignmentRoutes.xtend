...
class AssignmentRoutes extends BaseRoute {
	...
	def resetAssignmentsForStatus(String toStatus) {
		if (STATUS_EDITING.equalsIgnoreCase(toStatus) || STATUS_MARKING.equalsIgnoreCase(toStatus))
			assignment.find("status = ? or status = ?", STATUS_EDITING, STATUS_MARKING)?.forEach [ a |
